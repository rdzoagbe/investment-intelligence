...
class BacktestRequest(BaseModel):
    ...
    scores: list[float] = Field(min_length=2)
    initial_cash: float = Field(default=10000, gt=0)
    position_pct: float = Field(default=0.20, gt=0, le=1)
    buy_threshold: float = Field(default=85, ge=0, le=100)
    sell_threshold: float = Field(default=60, ge=0, le=100)
...
class BrokerApproval(BaseModel):
    symbol: str
    side: str
    quantity: float = Field(gt=0)
    order_type: str = "MARKET"
...
@app.get("/api/scanner")
def investment_scanner(limit: int = Query(default=20, ge=1, le=100), offset: int = Query(default=0, ge=0), market: str | None = Query(default=None), verdict: str | None = Query(default=None), min_score: float = Query(default=0, ge=0, le=100)) -> dict[str, object]:
    ranked = scanner(100)
    if market:
        ranked = [x for x in ranked if str(x["market"]).lower() == market.strip().lower()]
    if verdict:
        ranked = [x for x in ranked if str(x["verdict"]).upper() == verdict.strip().upper()]
...
def risk() -> dict[str, object]:
    return risk_metrics([0.004, -0.002, 0.006, 0.003, -0.004, 0.005, 0.002, -0.001])
...
def paper_order(request: PaperOrder) -> dict[str, object]:
    if not get_asset(request.symbol):
        raise HTTPException(status_code=404, detail="Unknown symbol")
    try:
        return paper_account.submit(request.symbol, request.side, request.quantity, request.price)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc


@app.post("/api/broker/preview")
def broker_preview(request: BrokerPreview) -> dict[str, object]:
    try:
        return gateway.preview(request.symbol, request.side, request.quantity, request.order_type).__dict__
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
